...
import ai.baby.servlets.Controller.Page;
...
// ------------------------ CANONICAL METHODS ------------------------
    ...
    protected void init(final SignInOnCriteria signInOnCriteria) {
        ...
        if (criteria.username.validate() == 0) {
            ...
            {
                final HumansIdentity hi = UserProperty.HUMANS_IDENTITY_CACHE.get(criteria.username.getObjectAsValid(), "");
                                            ...
                                            switch (criteria.getSignInOnDisplayComponent()) {
                                                case HOME: {
                                                    $$sendJS(JSCodeToSend.redirectPageWithURL(ProfileRedirect.PROFILE_URL
                                                            + UserProperty.HUMANS_IDENTITY_CACHE.get(criteria.username.getObjectAsValid(), "").getUrl().getUrl()));
                                                    break;
                                                }
                                                default: {
