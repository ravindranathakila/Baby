...
import ai.baby.logic.mail.SendMail;
...
final public class ServletSignup extends HttpServlet {
    ...
    protected void processRequest(final HttpServletRequest request__, final HttpServletResponse response__)
        ...
        logger.debug(logMsgs.getString("ServletSignup.0003"));
        while (enumerated.hasMoreElements()) {
            String param = (String) enumerated.nextElement();
            logger.debug(logMsgs.getString("ServletSignup.0001"), param);
            logger.debug(logMsgs.getString("ServletSignup.0002"), request__.getParameter(param).length());
            ...
            validation:
            {
                validate_username:
                {
                    ...
                    if (username == null || username.equals(EMPTY) || username.equals(JS_NULL)) {/*username*/
                        ...
                        userSession_.setAttribute(UsernameErrorMsg, gUI.getString("ServletSignup.0002"));
                        allParametersNotOkFlag = Boolean.TRUE;
                    } else if ((new Email(username).validate()) != 0) {
                        userSession_.setAttribute(UsernameError, ErrorStatusTrue);
                        userSession_.setAttribute(UsernameErrorMsg, gUI.getString("ServletSignup.0007"));
                        allParametersNotOkFlag = Boolean.TRUE;
                    } else if (DB.getHumanCRUDHumanLocal(true).doDirtyCheckHuman(username).returnValue()) {
                        userSession_.setAttribute(UsernameError, ErrorStatusTrue);
                        userSession_.setAttribute(UsernameErrorMsg, gUI.getString("ServletSignup.0003") + username);
                        allParametersNotOkFlag = Boolean.TRUE;
                    } else {
                        logger.info(RBGet.logMsgs.getString("ServletSignup.0006"));
                    }
                }
                ...
                validate_password:
                {
                    ...
                    if (password == null || password.equals(EMPTY) || password.equals(JS_NULL)) {/*password*/
                        ...
                        userSession_.setAttribute(PasswordError, ErrorStatusTrue);
                        userSession_.setAttribute(PasswordErrorMsg, gUI.getString("ServletSignup.0008"));
                        allParametersNotOkFlag = Boolean.TRUE;
                    } else {
                        logger.info(RBGet.logMsgs.getString("ServletSignup.0007"));
                    }
                }
            }
...
//                        response__.sendRedirect(Page.Activate.getURL());
                    ...
                    final Throwable e) {
                userSession_.setAttribute(UsernameError, ErrorStatusTrue);
                userSession_.setAttribute(UsernameErrorMsg, gUI.getString("ServletSignup.0001") + username);
                request__.getRequestDispatcher("/signup.jsp").forward(request__, response__);
                break processSignup;
            }
